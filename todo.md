...
- Design spec done: `docs/superpowers/specs/2026-08-14-written-media-design.md` — new `written_media` table (separate from `shows`, pages for books / chapters+volumes for manga-manhwa-webnovel), a 5th "Reading" tab, `ratings` table extended via a nullable second FK to cover both content types. Tier List/Ratings-tab UI/Stats breakdown for reading explicitly deferred to a later round. Next step: run it through `writing-plans` for an implementation plan.
